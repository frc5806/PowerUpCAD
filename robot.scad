...
module full_chassis() {
    ...
    color([0.2,0.2,0.2]) {
        translate([(CHASSIS_WIDTH+CHASSIS_CUTOUT_WIDTH)/2-BOX_TUBE_WIDTH-.25,BOX_TUBE_WIDTH+0.25,(WHEEL_DIAMETER-BOX_TUBE_HEIGHT)/2-WHEEL_CENTER_OFFSET]) if(COMPRESSOR_TYPE == "little") little_compressor();
        else big_compressor();
    }
    
    // Add in air tanks, too (config 1)
    color([0.4,0.4,0.4]) {
        translate([(CHASSIS_WIDTH-CHASSIS_CUTOUT_WIDTH)/2+.25,BOX_TUBE_WIDTH+.25+2.75/2,BASE_CHASSIS_HEIGHT]) tank();
        translate([(CHASSIS_WIDTH-CHASSIS_CUTOUT_WIDTH)/2+.25,BOX_TUBE_WIDTH+.5+3*2.75/2,BASE_CHASSIS_HEIGHT]) tank();
    }
